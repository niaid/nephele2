---
title: "Pre-processing QC Pipeline"
output:
  html_document:
    md_extensions: +raw_html-smart+definition_lists-tex_math_single_backslash
    mathjax: null
    section_divs: false
    smart: false
    highlight: tango
    theme: null
    template: ../../resources/Rdocs/user_guide_template.html
...
is_html_output = function(print=F) {
  out.format <- knitr::opts_knit$get("rmarkdown.pandoc.to")
  if(print) {
    print(out.format)
  }

  if (!is.null(names(out.format))) return(grepl("html", names(out.format)))
  return(grepl("html", out.format))
}
```
...
- Nephele uses [FastQC](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/){target="_blank" rel="noopener noreferrer"} v0.11.7. 

### 2. Adapter/Primer trimming
- Adapter and/or primer trimming is *optional*.
- Nephele uses the [cutadapt plugin](https://docs.qiime2.org/2018.6/plugins/available/cutadapt/){target="_blank" rel="noopener noreferrer"} from QIIME 2 version 2018.6 for primer and adapter trimming.  
...
- More information on adapter and primer trimming can be found in the [QIIME 2 docs](https://docs.qiime2.org/2018.6/plugins/available/cutadapt/){target="_blank" rel="noopener noreferrer"}, and on [cutadapt's help page](https://cutadapt.readthedocs.io/en/stable/){target="_blank" rel="noopener noreferrer"}.

### 3. Quality trimming
- Quality trimming is *optional*.
- [Trimmomatic](http://www.usadellab.org/cms/?page=trimmomatic){target="_blank" rel="noopener noreferrer"} v0.38 is used for quality trimming.
...
- Poor quality sequence can also be trimmed at the start of a read using the [`Leading quality`](#quality-trimming-1) trim option.
- Further help can be found in the [Trimmomatic manual](http://www.usadellab.org/cms/uploads/supplementary/Trimmomatic/TrimmomaticManual_V0.32.pdf){target="_blank" rel="noopener noreferrer"}.

### 4. Paired-end read merging
- Read merging is only for paired-end data sets and is *optional*.
- For merging read pairs, Nephele uses [FLASH2](https://github.com/dstreett/FLASH2){target="_blank" rel="noopener noreferrer"} v2.2.00 which is based on the [FLASH](https://ccb.jhu.edu/software/FLASH/){target="_blank" rel="noopener noreferrer"} read merger.
- FLASH is designed to merge pairs of reads when the original fragment length is shorter than twice the length of reads.  It merges read pairs if the rate of mismatches in the overlapping region is less than the user-specified [`Maximum mismatch density`](#merge-read-pairs).  See user options below for more information.

### 5. Summary graphs
- Summary graphs are *always made* as the final step of the pipeline.
- Nephele uses [MultiQC](http://multiqc.info/){target="_blank" rel="noopener noreferrer"} v1.8dev.  
...
```{asis screenshot_html, eval=is_html_output(), echo=is_html_output()}
  <div>
    <img class="img-responsive" src="{{ url_for('static', filename='images/outie.svg') }}" alt="outie orientation" width="30%">
  </div>
```

```{r screenshot, out.width="30%", echo=F, eval=!is_html_output()}
    knitr::include_graphics("../../nephele/static/images/outie.svg")
```
...
- **multiqc_input**
  ...
  - *\*fastqc.html/.zip*: FastQC reports.  See the [FastQC Analysis module docs](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/Help/3%20Analysis%20Modules/){target="_blank" rel="noopener noreferrer"} for more information.
- *multiqc_report.html*
  : [MultiQC HTML report](http://multiqc.info/docs/#using-multiqc-reports){target="_blank" rel="noopener noreferrer"} with summary graphs.
- **multiqc_data**
  : files containing [parsed data](http://multiqc.info/docs/#parsed-data-directory){target="_blank" rel="noopener noreferrer"} output by MultiQC.
- **qtrimmed_seqs**
...
@. Andrews, S. Babraham Bioinformatics - FastQC A Quality Control tool for High Throughput Sequence Data. (n.d.). Retrieved September 11, 2018, from <https://www.bioinformatics.babraham.ac.uk/projects/fastqc/>{target="_blank" rel="noopener noreferrer"}.
@. Martin, M. (2011). Cutadapt removes adapter sequences from high-throughput sequencing reads. *EMBnet.Journal*, *17*(1), 10-12. doi: [10.14806/ej.17.1.200](https://doi.org/10.14806/ej.17.1.200){target="_blank" rel="noopener noreferrer"}.
...
@. Bolger, A. M., Lohse, M., & Usadel, B. (2014). Trimmomatic: a flexible trimmer for Illumina sequence data. *Bioinformatics*, *30*(15), 2114. doi: [10.1093/bioinformatics/btu170](https://doi.org/10.1093/bioinformatics/btu170){target="_blank" rel="noopener noreferrer"}.
@. Magoc, T., & Salzberg, S. L. (2011). FLASH: fast length adjustment of short reads to improve genome assemblies. *Bioinformatics*, *27*(21), 2957-2963. doi: [10.1093/bioinformatics/btr507](https://doi.org/10.1093/bioinformatics/btr507).
@. Streett, D. (2018). *Flash2 has some improvements from flash_1 including new logic from innie and outie overlaps as well as some initial steps for flash for amplicons: dstreett/FLASH2*. C. Retrieved from <https://github.com/dstreett/FLASH2>{target="_blank" rel="noopener noreferrer"} (Original work published 2015).
@. Ewels, P., Magnusson, M., Lundin, S., & K&auml;ller, M. (2016). MultiQC: summarize analysis results for multiple tools and samples in a single report. *Bioinformatics*, *32*(19), 3047-3048. doi: [10.1093/bioinformatics/btw354](https://doi.org/10.1093/bioinformatics/btw354){target="_blank" rel="noopener noreferrer"}.
