...
import nephele2.pipelines.QIIME.join_read as join_read
...
def main(args):
    ...
    # join the paired end samples
    with open(pipe.args.map_file.name, "r") as f:
        reader = csv.DictReader(f, delimiter = "\t")
        for r in reader:
            pipe.check_file(os.path.join(pipe.inputs_dir, r['ForwardFastqFile']), "Forward sample")
            pipe.check_file(os.path.join(pipe.inputs_dir, r['ReverseFastqFile']), "Reverse sample")

    pe = join_read.join_read(pipe.log.info, pipe.args.min_overlap, pipe.args.perc_max_diff, pipe.inputs_dir, pipe.outputs_dir)
...
if __name__ == "__main__":
    ...
    PARSER.add_argument("--data_type", type = str, default = "PE")
    PARSER.add_argument("--map_file", type = argparse.FileType('r'), required = True)
    PARSER.add_argument('--min_overlap', type = int, default = 10,
        help='minimum allowed overlap in bases required to join pairs')
    PARSER.add_argument('--perc_max_diff', type = int, default = 25,
        help='fastq-join only; maximum allowed percent differences within overlap')
